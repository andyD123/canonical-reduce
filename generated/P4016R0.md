---
title: "P4016R0 — Canonical Parallel Reduction: A Fixed Expression Structure for Run-To-Run Consistency"
subtitle: "A Fixed Expression Structure for Run-To-Run Consistency"
---

<div class="wg21-header">
...
When evaluated in a constant-evaluation context, the returned value is as-if obtained by evaluating the canonical abstract expression defined in §4. Constant evaluation does not require parallelism; evaluations are sequenced as specified in §4 (deterministic traversal for non-policy evaluation). The expression is a pure functional specification with no dependence on runtime state, so it is structurally amenable to constant evaluation provided `binary_op` is itself usable in constant expressions. Detailed `constexpr` specification (including iterator constraints) is deferred to the API design paper.
...
**NVIDIA CUB (CCCL v3.1, December 2025):** NVIDIA's CUDA Core Compute Libraries now provide an explicit determinism API with three levels: `cuda::execution::determinism::not_guaranteed` (maximum throughput, arbitrary reassociation), `determinism::run_to_run` (bitwise identity on the same GPU architecture; defaults are version-specific), and `determinism::gpu_to_gpu` (bitwise identity across GPU architectures). Users select determinism via a composable environment: `auto env = cuda::execution::require(cuda::execution::determinism::run_to_run);`. See: NVIDIA CUB API Documentation [NvidiaCUB].
...
- `std::reduce` explicitly permits reassociation. Therefore, even with a deterministic view and a fixed input order, `std::reduce` may legally evaluate a different abstract expression (see [reduce]).
...
Both algorithms have comparable pairwise-summation error bounds (see [Higham2002]). Both produce identical trees for power-of-two k. The practical impact is limited to:
...
Both trees have depth 3 (= ⌈log₂ 7⌉). Both perform exactly 6 applications of `binary_op`. The error bounds are comparable (see [Higham2002]). The only difference is which element carries: IPR carries `e₆` (the last element, locally determined), while recursive bisection splits at `floor(7/2) = 3`, changing the pairing of `e₀` (a globally determined property — the pairing of the first element depends on the total count).
...
- **Without an execution policy**, evaluations of `binary_op` are sequenced in the deterministic order induced by a left-to-right postorder traversal of the canonical expression tree (evaluate the left subexpression to completion, then the right subexpression to completion, then apply `binary_op` at the parent node).
- **With an execution policy**, the relative order of evaluations of `binary_op` — and thus the relative order of any side effects — is unspecified; evaluations may be unsequenced or concurrent, as permitted by `[algorithms.parallel.exec]`, provided the returned value matches that of the canonical expression.
...
```text
              COMBINE          → propagates left child (right is ∅)
             /       \
...
**Execution independence:** The returned value is determined solely by the canonical expression (§4). Without an execution policy, evaluations are sequenced in the deterministic order induced by the canonical tree traversal specified in §4. With an execution policy, the relative order of evaluations of `binary_op` is unspecified and evaluations may be unsequenced or concurrent, as permitted by `[algorithms.parallel.exec]`.
...
**Relationship to `std::simd` (P1928):** The canonical expression defined in §4 is designed to compose naturally with SIMD execution. The interleaved topology maps directly onto vertical operations over `basic_vec` values: each contiguous load feeds one element per lane, and the canonical tree over each lane's subsequence corresponds to successive vertical accumulations. An implementation may use `std::simd` operations to evaluate the canonical expression efficiently; the semantic contract does not depend on `std::simd` but is designed to be SIMD-friendly.
...
**Standard specification models for unordered summation.** The Standard defines two recursive summation models in [numerics.defns] (N5032) that differ in how much freedom the implementation has:
...
The canonical reduction expression proposed here fixes both degrees of freedom: operand ordering is determined by interleaved lane assignment (§4), and parenthesisation is determined by the balanced binary tree (§4.1). For a given lane count `L`, there is exactly one conforming abstract expression.
...
Kokkos is a widely used C++ programming model for exascale computing. Kokkos guarantees that repeated executions of a `parallel_reduce` or `parallel_scan` will produce bitwise-identical results provided the hardware resources and run-time settings remain constant [KokkosReduce]. However, this guarantee does not extend across different thread counts, backend configurations, or architectures — changing any of these may change the library-internal reduction tree, producing different floating-point results for non-associative operations.

In practice, HPC users who need reproducibility across configurations (e.g., scaling studies, regression testing across cluster generations) must implement workarounds such as fixed-topology reductions or fixed-point arithmetic, which imposes a significant developer burden. The interleaved topology proposed in §4.3 of this paper mirrors the "vector-lane" optimization patterns used in Kokkos's backend, but elevates it to a standardized semantic contract whose expression is determined by the user-chosen `L`, not by the runtime environment.

### C.2 Intel oneAPI Threading Building Blocks (oneTBB)

oneTBB is a widely used library for task-parallel programming on CPUs. It provides two reduction facilities:

**`parallel_reduce`** uses a work-stealing scheduler with a recursive splitting strategy: the range is split into sub-ranges, each sub-range is reduced locally, and sub-results are combined using the user-provided `join()` method. Because work-stealing determines which thread becomes free first, the order in which `join()` operations occur is schedule-dependent — different runs may build different reduction trees, producing different floating-point results for non-associative operations.

**`parallel_deterministic_reduce`** addresses this by building a deterministic reduction graph: the split/join structure is fixed regardless of scheduling, so repeated executions with the same range and partitioner produce bitwise-identical results [IntelTBB]. However, the tree shape is library-defined (determined by the partitioner and grain size), not standardized or user-specified. Changing the thread count, grain size, or partitioner may change the tree and thus the result. The key contrast with the canonical reduction proposed in §4 is that `parallel_deterministic_reduce` guarantees a *stable* tree for a given configuration, whereas this proposal specifies a *standard-defined* tree determined solely by the user-chosen topology coordinate `L` — independent of thread count, partitioner, or runtime environment.

### C.3 NVIDIA Thrust and CUB

For GPU architectures, NVIDIA provides the Thrust and CUB libraries.

**Thrust:** Generally favors throughput; depending on backend and algorithm, implementations may use atomic operations or other schedule-dependent techniques for reductions. As a result, floating-point results may be non-deterministic across runs.

**CUB:** Provides more granular control through "Block-level" and "Warp-level" primitives. The interleaved subsequences (S_j) defined in §4.3 are mathematically equivalent to the "striped-arrangement" memory pattern used in CUB to achieve peak bandwidth on SIMD-intensive hardware [NvidiaCUB].
...
- Consistent with the reduce family having an explicit `init` parameter; this paper additionally fixes init placement to a single final `op(I, R)` to keep the canonical tree independent of `init` ([reduce]).
