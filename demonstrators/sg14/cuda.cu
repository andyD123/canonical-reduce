...
// ============================================================================
// atomicAdd for double fallback (Godbolt sometimes disables native atomicAdd(double*))
// ============================================================================
    ...
    int N)
{
    ...
    constexpr int MAX_PER_LANE = CHUNK_SIZE / L;            // 256 or 32
    constexpr int THREADS_PER_LANE = BLOCK_THREADS / L;     // 16 or 2
    ...
    int tid = threadIdx.x;
    int lane = tid % L;
    int lane_local = tid / L; // 0..THREADS_PER_LANE-1
    ...
    // NaN sentinel for partial tail detection
    double vals[16];
    #pragma unroll
    for (int t = 0; t < 16; ++t) {
        int p   = base_p + t;
    ...
    double seg_sum;
    if (active == 16) {
        // exact balanced tree for 16 (deterministic)
        double tmp[16];
        #pragma unroll
        ...
        // exact tree_host for tail prefix
        double tmp[16];
        #pragma unroll
        for (int i = 0; i < 16; ++i) tmp[i] = (i < active) ? vals[i] : 0.0;
        seg_sum = tree16_exact(tmp, active);
    } else {
        seg_sum = __longlong_as_double(0x7ff8000000000000ULL); // empty => NaN
    }

    __shared__ double segs[256]; // fits (16*16 or 2*128 both = 256)
...
// Fix: ping-pong (double-buffer) shared memory to remove in-place race.
// Shared memory required: 2*num_blocks doubles.
// ============================================================================
template<int L>
__global__ void phase2_single_kernel(
    const double* __restrict__ block_lanes, // [num_blocks][L]
    double* __restrict__ final_lanes,       // [L]
    int num_blocks)
{
    int lane = blockIdx.x;
    int tid  = threadIdx.x;

    extern __shared__ double sh[]; // 2*num_blocks doubles
    double* src = sh;
...
// ============================================================================
// Phase 3 (CANONICAL): final fold over L lanes (iterative pairwise + carry)
// Done in thread 0 for determinism/auditability; L<=128 here.
// ============================================================================
template<int L>
                          ...
                          int N)
{
    ...
    // 2x shared memory for ping-pong
...
// ============================================================================
// Reproducibility stress tests (length sweep + hostile cancellation)
// ============================================================================
                                 ...
                                 uint64_t seed)
{
    auto run_one = [&](int L, size_t N, bool hostile) -> bool {
        ...
        // Stability (repeat a few times; should be identical)
        bool stable = true;
        for (int r = 0; r < 3; ++r) {
            double gpu2 = (L == 16)
                ? run_canonical_fast<16>(d_data, d_block_lanes, d_final_lanes, d_out, (int)N)
    ...
    // lengths: small awkward, around L, and around block boundaries.
    ...
    // around lane counts (even though L fixed here, still useful)
    add_around(16);
    add_around(128);

    // around chunk/block boundaries (CHUNK_SIZE=4096)
    add_around(4096);
    add_around(8192);
    add_around(12288);
...
int main() {
    ...
    // Host reference (canonical cross-lane)
    ...
    // Device buffers (allocate once; reuse for sweep)
    ...
    void* d_temp = nullptr;
    size_t temp_bytes = 0;
    cub::DeviceReduce::Sum(d_temp, temp_bytes, d_data, d_out, N); // size query
    CUDA_CHECK(cudaMalloc(&d_temp, temp_bytes));
#endif

    // Correctness on big N
    ...
    // Extra reproducibility tests (length sweep + hostile cancellation)
    bool sweep_ok =
